...
// ─── Helper functions for batch upserts ───────────────────────────────────

// Helper function to upsert a batch of goals
async function _upsertGoalBatch(goals, userId) {
    const values = [];
...
// Processes goals in sub-batches of 50 to avoid hitting query size limits
router.post('/goals/bulk', async (req, res, next) => {
    try {
        ...
        for (let i = 0; i < goals.length; i += BATCH_SIZE) {
            ...
            const upserted = await _upsertGoalBatch(batch, req.user.sub);
            totalUpserted += upserted;
        }

        res.json({ upserted: totalUpserted });
    } catch (err) {
        next(err);
    }
});
...
// Processes tasks in sub-batches of 50 to avoid hitting query size limits
router.post('/bulk', async (req, res, next) => {
    try {
        ...
        for (let i = 0; i < tasks.length; i += BATCH_SIZE) {
            ...
            const upserted = await _upsertTaskBatch(batch, req.user.sub);
            totalUpserted += upserted;
        }
