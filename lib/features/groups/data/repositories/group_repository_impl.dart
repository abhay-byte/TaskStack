...
class GroupRepositoryImpl implements GroupRepository {
  ...
  Future<Group> createGroup({
    ...
    final memberRef =
        _firestore.collection('group_members').doc('${groupId}_$_uid');
    ...
    // Write invite code to Realtime Database for O(1) lookup
  ...
  Future<void> joinByCode(String inviteCode) async {
    // Lookup groupId from Realtime Database
    final snapshot = await _rtdb.ref('inviteCodes/$inviteCode').get();
    if (!snapshot.exists) throw Exception('Invalid invite code.');
    final groupId = snapshot.value as String;
